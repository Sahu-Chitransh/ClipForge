...
class DownloadItem(BaseModel):
    ...
    audio_bitrate: str | None = Field(default=None, alias="audioBitrate")
    trim_segment: bool | None = Field(default=None, alias="trimSegment")
...
async def startup_legacy_runtime() -> None:
    global cleanup_task, main_loop
    main_loop = asyncio.get_running_loop()
    cleanup_task = asyncio.create_task(cleanup_loop())


async def shutdown_legacy_runtime() -> None:
    global cleanup_task, main_loop
    task = cleanup_task
    cleanup_task = None
    main_loop = None
...
def resolve_video_format(quality: str | None) -> str:
...
@legacy_router.post("/api/download")
...
@legacy_router.post("/api/metadata")
...
@legacy_router.post("/api/playlist")
...
@legacy_router.get("/api/status/{job_id}")
...
@legacy_router.get("/api/files/{job_id}")
